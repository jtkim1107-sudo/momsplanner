...
// 브랜드칩 · 별똥별) 재사용.
// ⚠️ 기준은 '판정' — 아래 공유 리스트 3종은 후보 풀(학습 자료)이고,
// 판정을 거쳐 판정템으로 확정된 것이 스탠다드다. 병합된 원자료:
//  · 조리원동기맘 · 산후8주맘 (기존)
...
function renderPostpartum(){
  ...
  if(ppMode==='std'){
    ...
    intro.innerHTML = `
      ...
      <div class="ss-chips"><span>판정템 ${totalAll}</span><span>판정 데이터 기반</span><span>원자료: 실제 가방 3종</span></div>
    `;
  }else{
