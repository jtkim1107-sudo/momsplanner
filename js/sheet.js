...
// 소행성 육아플래너 — 출산 준비물 시트
// ------------------------------------------------------------
// ⚠️ 기준은 '판정'이다. 아래 선배맘 공유 자료 4종은 기준이 아니라
// 후보 풀(학습 자료) — 여기 항목들이 본체 앱 판정을 거쳐 '판정템'으로
// 확정되면 스탠다드에 오른다. 자료는 계속 추가 공급됨.
// 병합된 원자료:
...
function renderSheet(){
  ...
  if(sheetMode==='std'){
    ...
    intro.innerHTML = `
      ...
      <div class="ss-chips"><span>판정템 ${cnt.std}</span><span>판정 데이터 기반</span><span>원자료: 공유 리스트 4종</span></div>
    `;
  }else{
